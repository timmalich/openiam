...
/**
 * Performance test for the Accessor entity.
 */
class AccessorGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the Accessor entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all accessors")
            .get("/api/accessors")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new accessor")
            .post("/api/accessors")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
            ...
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_accessor_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created accessor")
                .get("${new_accessor_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created accessor")
            .delete("${new_accessor_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
