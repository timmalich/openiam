...
/**
 * Performance test for the Entitlement entity.
 */
class EntitlementGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the Entitlement entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all entitlements")
            .get("/api/entitlements")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new entitlement")
            ...
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_entitlement_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created entitlement")
                .get("${new_entitlement_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created entitlement")
            .delete("${new_entitlement_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
