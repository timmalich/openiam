...
import com.malichzhang.openiam.config.KafkaProperties;
...
class OpeniamKafkaResourceIT {
    ...
    void setup() {
        ...
        OpeniamKafkaResource kafkaResource = new OpeniamKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/openiam-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/openiam-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
