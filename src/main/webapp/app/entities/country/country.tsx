  ...
  return (
    <div>
      <h2 id="country-heading">
        <Translate contentKey="openiamApp.country.home.title">Countries</Translate>
        <Link to={`${match.url}/new`} className="btn btn-primary float-right jh-create-entity" id="jh-create-entity">
          <FontAwesomeIcon icon="plus" />
          &nbsp;
          <Translate contentKey="openiamApp.country.home.createLabel">Create new Country</Translate>
        </Link>
      </h2>
      <Row>
        <Col sm="12">
            ...
            <AvGroup>
              <InputGroup>
                <AvInput
                  ...
                  placeholder={translate('openiamApp.country.home.search')}
                />
        ...
        {countryList && countryList.length > 0 ? (
            ...
            <tbody>
              {countryList.map((country, i) => (
                <tr key={`entity-${i}`}>
                  ...
                  <td>{country.accessor ? <Link to={`accessor/${country.accessor.id}`}>{country.accessor.id}</Link> : ''}</td>
                  <td>
                    {country.organization ? <Link to={`organization/${country.organization.id}`}>{country.organization.id}</Link> : ''}
                  </td>
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="openiamApp.country.home.notFound">No Countries found</Translate>
            </div>
          )
        )}
      </div>
